...
open class TernaryCheckboxTree(
    ...
    override var representedObject: TernaryCheckboxData
            by observing(TernaryCheckboxData(text = text, state = generateStateBasedOnChildren()),
        didSet = { old, new ->
            representedObjectChangeObservers.forEach {
                it(old, new)
            }
        }
    )
     ...
     */
    private val representedObjectChangeObservers = mutableListOf<OnRepresentedObjectChange<TernaryCheckboxData>>()
...
//        representedObjectChangeObservers.forEach { it(oldState, newState) }
//    })


    init {
        children.forEach {
            it.onRepresentedObjectDidChange { _, _ -> updateStateBasedOnChildren() }
        }
    ...
    override fun onRepresentedObjectDidChange(onRepresentedObjectChange: OnRepresentedObjectChange<TernaryCheckboxData>) {
        representedObjectChangeObservers.add(onRepresentedObjectChange)
    }
    ...
    private fun updateStateBasedOnChildren() {
        ...
        state = generateStateBasedOnChildren()
        shouldUpdateChildrenBasedOnState = true
    }


    private fun generateStateBasedOnChildren() = when (FiniteAmountSummary(children)) {
        is all -> `true`
        is none -> `false`
    ...
    open fun deinit() {
        this.representedObjectChangeObservers.clear()
    }
}
